...
const rows: { label: string; get: (p: ReturnType<typeof getProductById> & {}) => string }[] = [
  ...
  { label: 'Расход', get: (p) => p!.fuelConsumption },
  {
    label: 'Габариты, мм',
    get: (p) =>
      `${formatNumber(p!.dimensions.length)} × ${formatNumber(p!.dimensions.width)} × ${formatNumber(
        p!.dimensions.height
      )}`,
  },
            ...
            <thead>
              <tr className="bg-slate-50">
                ...
                {items.map((p) => (
                  <th
                    key={p.id}
                    className="px-4 py-4 text-left text-base font-semibold text-industrial-900"
                  >
              ...
              {rows.map((r) => (
                <tr key={r.label} className="border-t border-slate-200">
                  ...
                  {items.map((p) => (
                    <td
                      key={p.id}
                      className="px-4 py-3 font-mono text-industrial-900"
                    >
                      {r.get(p)}
                    </td>
                  ))}
